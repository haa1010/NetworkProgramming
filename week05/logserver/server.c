...
void processMsg(Client *client, char *data);
    ...
    while (1) {
        ...
        for (i = 0; i < MAX_CLI; i++) {
            ...
            if (FD_ISSET(sd, &readfds)) {
                ...
                processMsg(&clientList[i], buffer);
            }
...
void processMsg(Client *client, char *data) {
    ...
    switch (getMode(data)) {
        ...
        case MSG: {
            ...
            if (access(client->name, F_OK) != -1) {
                // exist
                fd = fopen(client->name, "a");
            } else {
                // not exist
                fd = fopen(client->name, "w");
            }


            if (fd == NULL) {
                perror("process msg fd == NULL");
                exit(12);
            }
